...
export const ProjectProgressCard = ({ stats }: { stats: ProjectStatisticsSummary }) => {
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-base font-bold text-gray-900 dark:text-white">Timeline Progress</h3>
      ...
      {stats.estimatedCompletion && (
        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center text-xs text-gray-500">
          ...
          Est. Completion: <span className="font-medium text-gray-900 dark:text-white ml-1">{new Date(stats.estimatedCompletion).toLocaleDateString()}</span>
        </div>
      )}
    </div>
  );
